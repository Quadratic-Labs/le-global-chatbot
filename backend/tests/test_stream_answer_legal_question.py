...
Fixtures (_build_hit, _build_metrics, FakeGenerationClient from
tests/support/rag_fixtures.py, and the exact initial/repaired answer
text pairs used below) are the same established, PROVEN-correct
fixtures test_rag_answer.py itself uses - real text known to trigger
the real validators the way the test names claim, not invented text
