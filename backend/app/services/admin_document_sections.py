...
from app.services.document_mutation import (
    ...
    remove_top_level_topic,
    rename_top_level_topic,
...
def update_effective_section(
    *,
    ...
    with country_lock(
        ...
        is_rename = (
            trimmed_new_title is not None
            and normalize_topic_title(trimmed_new_title)
            != normalize_topic_title(old_legal_topic)
        )

        if is_rename:
            trimmed_new_title = _validate_new_section_title(
                title=trimmed_new_title,
                existing_topics=[
                    topic
                    for topic in existing_topics
                    if topic.legal_topic != old_legal_topic
                ],
                operation="section_update",
            )

        target_legal_topic = (
            trimmed_new_title if is_rename else old_legal_topic
        )
        ...
        try:
            pre_edit_snapshot = [
                ...
                if item["_source"].get("legal_topic") == old_legal_topic
            ]
        ...
        try:
            try:
                if is_rename:
                    rename_top_level_topic(
                        ...
                        country=country,
                        legal_topic=old_legal_topic,
                        new_title=trimmed_new_title,
            ...
            if is_rename:
                try:
                    reparsed_topics = locate_top_level_topics(
                        Document(temp_path),
                        country=country,
                    )
                ...
                reparsed_names = [
                    topic.legal_topic for topic in reparsed_topics
                ]

                if old_legal_topic in reparsed_names:
                    raise AdminDocumentSectionUpdateFailedError(
                        message=(
                            "The section's old title is still "
                            "present after renaming - no change was "
                            "saved."
                        )
                    )
                ...
                if reparsed_names.count(target_legal_topic) != 1:
                    raise AdminDocumentSectionUpdateFailedError(
                        message=(
                            "The section's new title could not be "
                            "uniquely identified after renaming - no "
                            "change was saved."
                        )
                    )
                ...
                if len(reparsed_names) != len(existing_topics):
                    raise AdminDocumentSectionUpdateFailedError(
                        message=(
                            "Renaming this section changed the "
                            "document's overall section structure - "
                            "no change was saved."
                        )
                    )
                ...
                if len(set(reparsed_names)) != len(reparsed_names):
                    raise AdminDocumentSectionUpdateFailedError(
                        message=(
                            "Renaming this section produced a "
                            "duplicate section title - no change was "
                            "saved."
                        )
                    )
            ...
            try:
                new_chunks = _build_topic_chunks(
                    temp_path=temp_path,
                    country=country,
                    legal_topic=target_legal_topic,
                    metadata=metadata,
                )
            ...
            if is_rename:
                try:
                    _delete_chunks_except(
                        ...
                        keep_chunk_ids=[],
                        context=(
                            f"document {document_id!r} rename cleanup"
                        ),
                    )
                    ...
                    _rollback_rename(
                        ...
                        document_id=document_id,
                        old_legal_topic=old_legal_topic,
                        new_legal_topic=target_legal_topic,
                        pre_rename_snapshot=pre_edit_snapshot,
                        error=error,
                        context="section rename",
                    )
...
def _verify_topic_absent(
    *,
    ...
    try:
        ...
        raise AdminDocumentSectionUpdateFailedError(
            message=(
                "The post-write invariant check could not read the "
                "search index."
            )
        ) from error
    ...
    if remaining:
        raise AdminDocumentSectionUpdateFailedError(
            message=(
                "Post-write invariant check failed: chunks are still "
                "present in the search index under a topic that "
...
def _rollback_rename(
    *,
    ...
    old_legal_topic: str,
    new_legal_topic: str,
    pre_rename_snapshot: list[dict[str, Any]],
    error: Exception,
    context: str,
    source_path: Path | None = None,
    original_source_bytes: bytes | None = None,
    ...
    try:
        _delete_chunks_except(
            ...
            filters=[
                {"term": {"document_id": document_id}},
                {"term": {"legal_topic": new_legal_topic}},
            ],
            keep_chunk_ids=[],
            context=f"document {document_id!r} rename rollback cleanup",
        )
    ...
    if not source_restored or not old_restored or not new_cleaned_up:
        raise AdminDocumentRollbackError(
            f"The {context} failed, and the rollback afterward was "
            "incomplete (source "
            + ("restored" if source_restored else "NOT restored")
            + ", old section "
            + ("restored" if old_restored else "NOT restored")
            + ", new section "
            + ("cleaned up" if new_cleaned_up else "NOT cleaned up")
            + ") - manual recovery is required."
        ) from error

    if isinstance(error, AdminDocumentSectionUpdateFailedError):
        # Already the right business-error type (the invariant-
        # failure case) - re-raise as-is, never re-wrapped.
        raise error
    ...
    # Anything else (e.g. a DocumentIndexingError from the old-topic
    # cleanup step) - wrap into the same reported business error, the
    # rollback having already restored a fully consistent state.
    raise AdminDocumentSectionUpdateFailedError(
        message=(
            f"The {context} could not be completed; the search "
            "index and source document were rolled back to their "
            "previous content."
        )
    ) from error
    ...
    *,
    ...
    with country_lock(
        ...
        if len(existing_topics) <= 1:
            raise AdminDocumentSectionLastRemainingError(
                document_id=document_id,
                section_id=section_id,
            )
        ...
        other_topics_before = {
            topic.legal_topic
            for topic in existing_topics
            if topic.legal_topic != legal_topic
        }
        ...
        try:
            ...
            raise AdminDocumentSectionUpdateFailedError(
                message=(
                    "The section's current content could not be "
                    "read from the search index before deleting."
                )
            ) from error
        ...
        try:
            try:
                ...
                raise AdminDocumentSectionUpdateFailedError(
                    message=(
                        f"The section could not be deleted: {error}"
                    )
                ) from error

            # Deletion never builds new chunks, so this call's own
            # return value is unused - only its validation (a genuine
            # DOCX, country unchanged) matters here.
            _validate_mutated_docx(
                temp_path=temp_path,
                country=country,
                expected_country_code=country_code,
                document_metadata=document_metadata,
            ...
            if legal_topic in reparsed_names:
                raise AdminDocumentSectionUpdateFailedError(
                    message=(
                        "The section is still present after "
                        "deleting - no change was saved."
                    )
                )
            ...
            if reparsed_names != other_topics_before:
                raise AdminDocumentSectionUpdateFailedError(
                    message=(
                        "Deleting this section changed other "
                        "sections in the document - no change was "
                        "saved."
                    )
                )
            ...
            try:
                _delete_chunks_except(
                    ...
                    keep_chunk_ids=[],
                    context=(
                        f"document {document_id!r} section delete"
                    ),
                )
                ...
                try:
                    _restore_section_snapshot(
                        ...
                        document_id=document_id,
                        legal_topic=legal_topic,
                        snapshot=pre_delete_snapshot,
                        bulk_chunk_size=DEFAULT_BULK_CHUNK_SIZE,
                    )

                except Exception as rollback_error:
                    raise AdminDocumentRollbackError(
                        "The section could not be removed from the "
                        "search index, and restoring it afterward "
                        "also failed - manual recovery is required."
                    ) from rollback_error
                ...
                raise AdminDocumentSectionUpdateFailedError(
                    message=(
                        "The section could not be removed from the "
                        "search index - no change was saved."
                    )
                ) from error
            ...
            try:
                ...
                _rollback_after_source_replace_failure(
                    ...
                    document_id=document_id,
                    legal_topic=legal_topic,
                    pre_mutation_snapshot=pre_delete_snapshot,
                    replace_error=replace_error,
                    context="section deletion",
                )
