"""
Durable, technical persistence of the current EFFECTIVE content of
any admin-edited legal document sections.

Mission "ORDER 5C": OpenSearch treats an edited section exactly like
any other section (section 2/28 - "SECTION = SECTION", no special
retrieval handling), but OpenSearch alone is never a durable record
of *which* content is currently effective for a section - a Reindex
that only re-parses the DOCX would silently revert every edit (section
3's own worked example). This module is that small, filesystem-only
technical record - never a second RAG source, never exposed to
retrieval, only ever read back to reconstruct the effective document
during Reindex/GET/restart.

One JSON file per document_id, stored INSIDE source_directory (never
as a sibling of it - mission "ORDER 5"'s own lesson: a sibling
location requires write access to source_directory's own PARENT,
which is not guaranteed in production; source_directory itself always
is, since uploads are written there).
"""
...
class SectionEditState:
    ...
    def to_json_dict(self) -> dict[str, object]:
        return {
            ...
            "sections": {
                section_id: {
                    "legal_topic": edit.legal_topic,
                    "section": edit.section,
                    "subsection": edit.subsection,
                    "content": edit.content,
                }
                for section_id, edit in self.sections.items()
            },
        }
    ...
    def from_json_dict(payload: object) -> "SectionEditState":
        if not isinstance(payload, dict):
            raise SectionStateError(
                "Section-edit state file must contain a JSON object."
            )
        ...
        if schema_version != _SCHEMA_VERSION:
            raise SectionStateError(
                "Unsupported section-edit state schema_version: "
                f"{schema_version!r}."
            )
        ...
        if (
            ...
            raise SectionStateError(
                "Section-edit state file is missing required fields."
            )
        ...
        for section_id, raw_edit in raw_sections.items():
            if not isinstance(raw_edit, dict):
                raise SectionStateError(
                    f"Section-edit entry {section_id!r} is malformed."
                )
            ...
            if (
                not isinstance(legal_topic, str)
                or not legal_topic
                or not isinstance(section, str)
                or not section
                or not (subsection is None or isinstance(subsection, str))
            ...
            sections[section_id] = SectionEdit(
...
def _state_directory(source_directory: Path) -> Path:
    return (
        source_directory
        / SECTION_STATE_DIRECTORY_NAME
        / SECTION_EDITS_SUBDIRECTORY_NAME
    )
...
def read_section_edit_state(
    source_directory: Path,
    document_id: str,
) -> SectionEditState | None:
    ...
    try:
        ...
        raise SectionStateError(
            f"Section-edit state file for {document_id!r} could not "
            "be read."
        ) from error
...
def write_section_edit_state_atomic(
    source_directory: Path,
